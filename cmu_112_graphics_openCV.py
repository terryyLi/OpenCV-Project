...
class App(object):
    ...
    def cameraFired(app):
        pass  # respond to camera events

    ####################################
    # Implementation:
    ####################################

    def __init__(app, width=300, height=300, x=0, y=0, title='Fruit Ninja By Terry', autorun=True, mvcCheck=True, logDrawingCalls=True):
    ...
    def _timerFiredWrapper(app):
        if (not app._running) or (
                not app._methodIsOverridden('timerFired') and app._methodIsOverridden('cameraFired')): return
        if (not app._paused):
            app.timerFired()

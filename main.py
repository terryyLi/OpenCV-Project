...
def appStarted(app):
    ...
    app.image2 = app.loadImage('assets/background.png')
    ...
    app.image4 = app.loadImage('assets/title.png')
    app.fruits = []
    app.timerDelay = 100
    app.gameStart = True
    ...
    # app.startX is the x coordinate of mouse on the splashscreen
    app.startX = None
    # app.startY is the x coordinate of mouse on the splashscreen
    app.startY = None
    # app.x is the x coordinate of mouse in game mode
    app.x = None
    # app.y is the y coordinate of mouse in game mode
    app.y = None
    app.r = 10
    app.time = 1
    app.cameraOpen = True
...
# mousemoved
# tracks the position of the purple tracker 
...
def cameraFired(app):
    ...
    if app.gameMode_1:
        # flips frame
        # frame is the video of myself accessed by camera
        # flip function flips a pic horitontally or
        # vertically based on the number code
...
def keyPressed(app, event):
    ...
    if event.key == "r":
        appStarted(app)
    # press b to go back to the splashscreen
...
def timerFired(app):
    ...
    gap = 30
    x = 200
    # total number of fruits on the screen
    total = random.randint(3, 5)
    # randomly generates some fruits to be thrown from bottom of screen
    if app.time == 15 or app.time % gap == 0:
        ...
        for i in range(total):
            ...
            # convert angle in degree to redian
            a = a * math.pi / 180
            # fruits are randomly appended to app.fruits list
            if n == 0:
                fruit = apple(v, a)
    ...
    for fruit in app.fruits:
        fruit.t += 1
        # finds the x and y coordinates for each fruit
        fruit.x += fruit.findX()
...
def redrawAll(app, canvas):
    ...
    # game mode
        ...
        canvas.create_text(220, 40, text=f"THIS MANY LIVES LEFT: {app.lives}", fill="yellow",
                           font="Arial 30 bold")
        # prints the number of score on  screen
        ...
        # creates mouse interaction in game
        ...
        for fruit in app.fruits:
            ...
            r = fruit.r
            if fruit.split:
                # if a bomb is splitted
                if isinstance(fruit, bomb):
                    # create a bomb getting bigger
                    r += 3
                    fruit.r = r
                ...
                try:
                    # draw the fruits or bomb following a projectile
                    canvas.create_oval(fruit.x - r, fruit.y - r, fruit.x + r, fruit.y + r, fill=c)
                    # draw the cross on bomb
                    if isinstance(fruit, bomb):
